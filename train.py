...
#########################################################################################
    ...
    dataset = AngleSteering(csv_file="",root_dir="",transform=transforms.ToTensor())

    train_set,test_set = torch.utils.data.random_split(dataset,[1028,400])

    train_loader = DataLoader(dataset = train_set,batch_size =args.batchSz, shuffle = True)
    test_loader = DataLoader(dataset = test_set,batch_size = args.batchSz, shuffle = True)
########################################################################################
